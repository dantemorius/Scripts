...
sed -i s/177.70.99.145/187.33.2.153/g /etc/init.d/firewall
sed -i s/177.70.99.149/187.33.2.154/g /etc/init.d/firewall
sed -i s/-SP/-RJ/g /etc/init.d/firewall

echo "$IPTABLES -A INPUT -s 177.70.99.145 -j ACCEPT
$IPTABLES -A INPUT -s 177.70.99.149 -j ACCEPT" >> $ARQNAT
sed -i '/# Firewalls Estruturas SP/ r '"$ARQNAT"'' /etc/init.d/firewall
rm -f $ARQNAT
