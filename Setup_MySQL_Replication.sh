...
ssh root@$MYSQL02 "rpm -ivh rpm -ivh http://dev.mysql.com/get/mysql-community-release-el6-5.noarch.rpm --force --force"
...
ssh root@$MYSQL02 "sed -i '27s/enabled=1/enabled=0/' /etc/yum.repos.d/mysql-community.repo"
...
sed -i '19s/enabled=0/enabled=1/g' /etc/yum.repos.d/mysql-community.repo && yum install openssh-clients.x86_64 rsync.x86_64 wget.x86_64 vim-X11.x86_64 vim-enhanced.x86_64 mlocate.x86_64 nc.x86_64 tcpdump telnet sshpass nc.x86_64 pwgen.x86_64 screen -y
...
ssh root@$MYSQL02 "sed -i '19s/enabled=0/enabled=1/g' /etc/yum.repos.d/mysql-community.repo && yum install openssh-clients.x86_64 rsync.x86_64 wget.x86_64 vim-X11.x86_64 vim-enhanced.x86_64 mlocate.x86_64 nc.x86_64 tcpdump telnet sshpass nc.x86_64 pwgen.x86_64 screen -y"
...
ssh root@$MYSQL02 "sed -i 's/=enforcing/=disabled/' /etc/selinux/config"
echo ""
...
ssh root@$MYSQL02 "/etc/init.d/iptables stop"
ssh root@$MYSQL02 "iptables -F"
ssh root@$MYSQL02 "chkconfig iptables off"

echo ""
echo -e "\\033[1;39m \\033[1;32mIPTABLES Parado.\\033[1;39m \\033[1;0m"
echo ""
...
ssh root@$MYSQL02 '/usr/bin/mysqladmin -u root password "$SENHA"'
